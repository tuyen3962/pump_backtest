...
func WriteReport(w io.Writer, res Result) {
	cfg := res.Config
	fmt.Fprintf(w, "=== Strategy v1 (whale_armed) ===\n")
	...
	fmt.Fprintf(w, "filter liq>=$%.0f vol1h>=$%.0f  skipped_entries=%d\n",
		cfg.MinLiquidityUSD, cfg.MinVolumeUSD1h, res.Skipped)
	fmt.Fprintf(w, "legs=%d coins=%d (closed_legs=%d open_legs=%d)  winrate=%.1f%%  avg_return=%.2f%%\n",
	...
	if len(res.Coins) == 0 {
		fmt.Fprintf(w, "(no trades — need whale_armed signals passing filters)\n")
		return
	}
